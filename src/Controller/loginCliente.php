...
include $_SERVER['DOCUMENT_ROOT'] ."/Projeto-Locadrive/src/Model/Login.php";
    ...
    if(isset($_POST['action']) == "login" and isset($_POST['email']) and isset($_POST['senha']) and !isset($_COOKIE['PHPSESSID'])){
        ...
        try {
            $sql = $connect->prepare("SELECT cliente.nome, cliente.email, cliente.privilegio FROM cliente WHERE cliente.email = '$email' AND cliente.senha = '$senha' ");
            $sql->execute();
            ...
            if(count($response) === 1){
                ...
                $cliente->logando();
                $_SESSION["user"] = array("nome" => $cliente->__get("nome"), "email" => $cliente->__get("email"), "privilegio" => $response[0]['privilegio']);
                echo "logado";
            }
            else{
                echo "houve um erro";
